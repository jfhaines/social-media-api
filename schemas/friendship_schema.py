...
class FriendshipSchema(ma.Schema):
    ...
    requester = fields.Integer(required=True, validate=Range(min=1, max=2, error='Incorrect input for requester'))
    status = fields.Integer(required=True, validate=Range(min=0, max=1, error='Incorrect input for status'))
